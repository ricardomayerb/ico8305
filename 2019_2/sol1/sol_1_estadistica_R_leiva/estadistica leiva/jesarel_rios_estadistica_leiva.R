...
# jesarel_rios_estadistica_leiva.R
# ejemplo:
...
media_pib <- mean(misdatos$pib_per_capita)
media_esv <- mean(misdatos$esperanza_de_vida)
# 2.2 la desviación estandar de cada una (4 pts)
sd_pib <- sd(misdatos$pib_per_capita)
sd_esv <- sd(misdatos$esperanza_de_vida)
...
hist_pib <- hist(misdatos$pib_per_capita)
hist_esv <- hist(misdatos$esperanza_de_vida)
hist_lg_pobl <- hist(log(misdatos$poblacion))
